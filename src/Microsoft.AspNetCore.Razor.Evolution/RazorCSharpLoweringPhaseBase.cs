...
namespace Microsoft.AspNetCore.Razor.Evolution
{
    internal abstract class RazorCSharpLoweringPhaseBase : RazorEnginePhaseBase, IRazorCSharpLoweringPhase
    {
        ...
        public class LineMappingBuilder : IDisposable
        {
            private readonly CSharpCodeWriter _writer;
            private readonly MappingLocation _sourceRange;
            private readonly SourceLocation _generatedLocation;

            public LineMappingBuilder(CSharpCodeWriter writer, MappingLocation sourceRange)
            {
            ...
            public void Dispose()
            {
                var generatedContentLength = _writer.Builder.Length - _generatedLocation.AbsoluteIndex;

                var generatedLocation = new MappingLocation(_generatedLocation, generatedContentLength);
                var documentMapping = _sourceRange;
                if (documentMapping.ContentLength == -1)
                {
                    documentMapping = new MappingLocation(
                        location: new SourceLocation(
                            _sourceRange.FilePath,
                            _sourceRange.AbsoluteIndex,
                            _sourceRange.LineIndex,
                            _sourceRange.CharacterIndex),
                        contentLength: generatedContentLength);
                }
